...
## 2026-03-17 (续 II-2) 採購推薦頁面連結修復 ✅ COMPLETE

### 🎯 推薦頁面連結導向和購物車功能修復

**目標**：修復推薦頁面連結導向錯誤，實現購物車功能
**時間**：2026-03-17 (下午)
**狀態**：✅ **完成** - 所有連結問題已修復

#### 🔧 修復問題
- ✅ **「查看詳情」按鈕修復**：改用 `useRouter` 同一標籤頁導航（避免 `window.open()` 導致重定向）
- ✅ **「加入採購單」功能實現**：調用 `/api/cart/items` 真正加入購物車
- ✅ **登入驗證**：加入購物車時檢查 401 Unauthorized，導向登入頁面
- ✅ **錯誤處理**：添加完整的網路錯誤和登入驗證邏輯

#### 📝 技術細節
- `RecommendationCard` 組件：修復導航邏輯，實現購物車 API
- 購物車請求：`POST /api/cart/items` 帶上 productId、quantity、supplierId
- 導航方式：使用 `router.push()` 代替 `window.open()` 確保一致的頁面流程
